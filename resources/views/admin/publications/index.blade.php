...
@can('publication_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            ...
            @include('csvImport.modal', ['model' => 'Publication', 'route' => 'admin.publications.parseCsvImport'])
        </div>
    </div>
@endcan
<div class="card">
                ...
                <tr>
                    ...
                    <td>
                        <select class="search" strict="true">
                            ...
                            @foreach(App\Models\Publication::EDITION_SELECT as $key => $item)
                    ...
                    <td>
                        <select class="search" strict="true">
                            <option value>{{ trans('global.all') }}</option>
                            @foreach(App\Models\Publication::DATABASE_SELECT as $key => $item)
                                <option value="{{ $key }}">{{ $item }}</option>
                            @endforeach
                        </select>
                    </td>
                    ...
                    <td>
                        <select class="search" strict="true">
                            <option value>{{ trans('global.all') }}</option>
                            @foreach(App\Models\Publication::TYPE_SELECT as $key => $item)
                                <option value="{{ $key }}">{{ $item }}</option>
                            @endforeach
                        </select>
                    </td>
