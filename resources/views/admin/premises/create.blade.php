...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.premise.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.premises.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('capacity') ? 'is-invalid' : '' }}" type="number" name="capacity" id="capacity" value="{{ old('capacity', '0') }}" step="1">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('type') ? 'is-invalid' : '' }}" name="type" id="type">
                    ...
                    @foreach(App\Models\Premise::TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('type', 'auditorium') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has('type'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('gps') ? 'is-invalid' : '' }}" name="gps" id="gps">{{ old('gps') }}</textarea>
                @if($errors->has('gps'))
                    <div class="invalid-feedback">
                        {{ $errors->first('gps') }}
                    </div>
            ...
            <div class="form-group">
                <label for="parent_id">{{ trans('cruds.premise.fields.parent') }}</label>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.premise.fields.parent_helper') }}</span>
            </div>
            <div class="form-group">
