    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.activities.update", [$activity->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('priority') ? 'is-invalid' : '' }}" type="number" name="priority" id="priority" value="{{ old('priority', $activity->priority) }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('author') ? 'is-invalid' : '' }}" name="author_id" id="author_id" required>
                    @foreach($authors as $id => $author)
                        <option value="{{ $id }}" {{ (old('author_id') ? old('author_id') : $activity->author->id ?? '') == $id ? 'selected' : '' }}>{{ $author }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('premise') ? 'is-invalid' : '' }}" name="premise_id" id="premise_id">
                    @foreach($premises as $id => $premise)
                        <option value="{{ $id }}" {{ (old('premise_id') ? old('premise_id') : $activity->premise->id ?? '') == $id ? 'selected' : '' }}>{{ $premise }}</option>
                    @endforeach
                </select>
