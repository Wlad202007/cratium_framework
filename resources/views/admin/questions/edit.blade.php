    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.questions.update", [$question->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('explanation') ? 'is-invalid' : '' }}" name="explanation" id="explanation">{!! old('explanation', $question->explanation) !!}</textarea>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('score') ? 'is-invalid' : '' }}" type="number" name="score" id="score" value="{{ old('score', $question->score) }}" step="0.1">
                @if($errors->has('score'))
                    <div class="invalid-feedback">
                        {{ $errors->first('score') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('activity') ? 'is-invalid' : '' }}" name="activity_id" id="activity_id" required>
                    @foreach($activities as $id => $activity)
                        <option value="{{ $id }}" {{ (old('activity_id') ? old('activity_id') : $question->activity->id ?? '') == $id ? 'selected' : '' }}>{{ $activity }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('priority') ? 'is-invalid' : '' }}" type="text" name="priority" id="priority" value="{{ old('priority', $question->priority) }}" required>
