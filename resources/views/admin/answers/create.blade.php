    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.answers.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('variant') ? 'is-invalid' : '' }}" name="variant_id" id="variant_id" required>
                    @foreach($variants as $id => $variant)
                        <option value="{{ $id }}" {{ old('variant_id') == $id ? 'selected' : '' }}>{{ $variant }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('long_answer') ? 'is-invalid' : '' }}" name="long_answer" id="long_answer">{!! old('long_answer') !!}</textarea>
