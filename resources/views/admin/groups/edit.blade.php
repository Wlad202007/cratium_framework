...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.groups.update", [$group->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label class="required" for="unit_id">{{ trans('cruds.group.fields.unit') }}</label>
                <select class="form-control select2 {{ $errors->has('unit') ? 'is-invalid' : '' }}" name="unit_id" id="unit_id" required>
                    @foreach($units as $id => $unit)
                        <option value="{{ $id }}" {{ (old('unit_id') ? old('unit_id') : $group->unit->id ?? '') == $id ? 'selected' : '' }}>{{ $unit }}</option>
                    @endforeach
                </select>
                @if($errors->has('unit'))
                    <div class="invalid-feedback">
                ...
                @endif
                <span class="help-block">{{ trans('cruds.group.fields.unit_helper') }}</span>
            </div>
            <div class="form-group">
                <label for="members">{{ trans('cruds.group.fields.members') }}</label>
                ...
                <select class="form-control select2 {{ $errors->has('members') ? 'is-invalid' : '' }}" name="members[]" id="members" multiple>
                    @foreach($members as $id => $members)
                        <option value="{{ $id }}" {{ (in_array($id, old('members', [])) || $group->members->contains($id)) ? 'selected' : '' }}>{{ $members }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('parent') ? 'is-invalid' : '' }}" name="parent_id" id="parent_id">
                    @foreach($parents as $id => $parent)
                        <option value="{{ $id }}" {{ (old('parent_id') ? old('parent_id') : $group->parent->id ?? '') == $id ? 'selected' : '' }}>{{ $parent }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('description') ? 'is-invalid' : '' }}" name="description" id="description">{!! old('description', $group->description) !!}</textarea>
            ...
            <div class="form-group">
                <label for="head_id">{{ trans('cruds.group.fields.head') }}</label>
                <select class="form-control select2 {{ $errors->has('head') ? 'is-invalid' : '' }}" name="head_id" id="head_id">
                    @foreach($heads as $id => $head)
                        <option value="{{ $id }}" {{ (old('head_id') ? old('head_id') : $group->head->id ?? '') == $id ? 'selected' : '' }}>{{ $head }}</option>
                    @endforeach
                </select>
                @if($errors->has('head'))
                    <div class="invalid-feedback">
                ...
                @endif
                <span class="help-block">{{ trans('cruds.group.fields.head_helper') }}</span>
            </div>
            <div class="form-group">
          ...
          return loader.file
            .then(function (file) {
              return new Promise(function(resolve, reject) {
                ...
                xhr.open('POST', '/admin/groups/ckmedia', true);
                ...
                data.append('crud_id', {{ $group->id ?? 0 }});
                xhr.send(data);
              });
            })
        }
